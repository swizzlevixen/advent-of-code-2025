...
for the_range in ranges:
    ...
    if match:
        ...
        for id_num in range(start_id, end_id + 1):
            ...
            # print(f" half of length {length} is {length // 2}")
            for i in range(1, length // 2 + 1):
                # print(f" Checking divisor {i} for ID {id_str}")
                # Check if evenly divisible
                if length % i == 0:
                    # print(f"  {i} divides evenly")
                    ...
                    while start_split + i <= length:
                        # print(
                        #     f"Checking ID {id_str}, segment size {i}, comparing {id_str[:i]}"
                        # )
